...
- **饮食** —— 我现在该吃什么。喝水、搜索记账、当天记录、当前饮食推荐。
  **搜索结果一行两个入口**：点行本身开份量面板（要改克数、选糖度走这条），
  点右边的 ＋ 直接进待记录篮子（`ui.basket`），用记住的份量，不开弹层。
  篮子只在页面内存里，按一次「记录到X餐」才落库 —— 一顿三菜一饭从 12 次操作降到 9 次。
...
- **数据** —— 今天同步了什么 + 在往哪走。健康数据（带图标）+ 「这一周」小结 + 一张趋势卡。
  小结在趋势图上面：九张单指标图各说各的，先回答「这一周整体怎么样」，
  想看某项怎么走再往下翻（`core/weekly-summary.js` 算，卡片只渲染）。
...
- **健身** —— 今天练什么、练了多少。
  ...
  **「已选动作」那张卡只做记录**：动作行、组数、重量、清空。覆盖部位和重复度
...
- **指标的性质决定画法，别再合回一根进度条**（`core/metrics.js`）：下限（蛋白、纤维）
  够了变绿；上限（钠、糖）接近变橙、超了变红；区间（热量、脂肪、碳水）落在里面就是对的；
  记录（饮水）**不画条**。区间条整条线性，罩子的位置只由区间本身决定——
  早先那版把区间压在 20%~80%，吃 10g 和吃 30g 的标记位置差不了多少，图在压缩事实。
  碳水从前归在「余数」里只报数，因为画一根没填满的条再写「还差 29g」，
  是界面在劝人多吃一个不存在的目标；现在它有自己的区间，没有「还差多少」这句话了。
...
- **减和增的速率上限不是同一个数**（`MAX_LOSS_RATE_PCT` 1% / `MAX_GAIN_RATE_PCT` 0.5%，
  ...
  计划上限、健康提示、图下解读三处必须从同一对常量取值，否则同一个应用
  对「多快算快」会给出两个答案。

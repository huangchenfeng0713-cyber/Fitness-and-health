...
- **饮水只数次数，不记毫升，也不画完成条**（`waterCount`）。饮料、汤、粥、水果和
  ...
  点一下加一次，误触了有「撤销一次」，健康说明收在旁边的感叹号里。
  Apple 健康同步来的 `waterMl` 照存不误，只是不拿来算进度。
- **碳水和脂肪在主卡上合用一条**（`macroSplit` + `splitBar`），显示两者按热量算的比例、
  结构偏向和各自的克数，条上一根竖标是计划的分界线。它们分的是同一块热量，
  ...
  参照物是这个人自己的计划分配，不是 AMDR；差 8 个百分点以内算「结构接近计划」
  （`SPLIT_BALANCED_PP`，护栏）。**结构偏移不画警告色**——怎么分本来就有很宽的
  合理区间，把「今天多吃了米饭」画成橙色是把偏好问题说成健康问题。

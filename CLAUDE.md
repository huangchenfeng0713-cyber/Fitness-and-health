...
- **健身** —— 今天练什么、练了多少。
  顶上是人体部位图（正反两张，`data/body-map.js` 只放形状）：点哪块选哪组，
  今日已选动作练到的部位会浅色点亮。挑动作本来就是「我今天想练这块」。
  动作行默认只给一行「主要练哪儿 · 什么模式」；协同肌收在「已选动作建议」那张卡里——
  真要看细节是在排计划的时候，不是在挑的时候。
  挑动作有两种入口，切换即可（`pickMode`）：**按部位**走人体图，
  **推拉腿**走 `core/training.js` 的 `SPLITS`（推 / 拉 / 腿 / 核心，按 `pattern` 归类）。
  推拉腿模式下不显示人体图 —— 那一版分法本来就不是按部位切的。
...
- 互斥的选择用分段控件（`.range-switch` / `.chart-switch`）：灰槽 + 选中格浮起一块白。

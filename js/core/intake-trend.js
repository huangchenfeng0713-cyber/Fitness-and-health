...
export function intakeTrend({ targets = {}, intake = {}, entries = [], rhythmEntries = [], now = new Date(), isToday = true, enabled = true } = {}) {
  ...
  const info = { ...base, remainingMeals: remainingMeals.map(m => ({ ...m, label: label(m.key) })) };
  ...
  if (todayEntries.some(e => new Date(e.time || '') > now)) return {
    ...info, reason: '今天含有未来时间的饮食条目，先核对哪些已经吃过，再判断全天走势。',
  };
  ...
  if (eaten > target + margin) {
    if (!remainingMeals.length && positive(targets.protein) - positive(intake.protein) <= 10) return {
      ...info, state: 'settled', reason: '主餐已记录完，今天不再追着差额调整，明天回到正常三餐。',
    };
  ...
  if (timedKcal < Math.max(400, target * 0.2) || Math.abs(timedKcal - eaten) > Math.max(50, eaten * 0.1)
    ...
    return { ...info, reason: '当前记录还不足以判断全天走势；先核对是否漏记，下一餐照常安排。' };
  }
  ...
  if (hour - latest < grace || insideMeal(hour)
    ...
    return { ...info, state: 'watch', reason: '餐次可能仍在进行或尚未记全，暂不提醒调整。' };
  }
  ...
  if (!remainingMeals.length) return { ...forecast, state: 'settled', reason: '今天的主餐已记录完；按饥饿感决定是否少量加餐，不必集中补齐差额。' };
...
export function correctionPlan({ trend, gaps, targets, hour }) {
  ...
  let action = '';
  if (active && trend.direction === 'under') {
    action = carbLow
  ...
  return { active, direction: trend.direction, protein, fiber, fatHigh, carbLow, carbHigh, lean, energyOver, optionalProtein, nextMeal, action };
}

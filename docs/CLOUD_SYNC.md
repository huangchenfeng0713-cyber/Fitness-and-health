...
- **版本没变不下载**（v3.24.3 起）：启动和每次本机修改后的同步先只读取云端的 revision 与更新时间；与本机一致时不再下载整份快照，本机有修改就直接按 revision 条件上传。云端更新过、云端没有这一行或需要冲突判断时，仍下载整份快照。每台设备升级后第一次启动仍会完整核对一次。同一账号打开应用时，界面只等账号归属确认，不再等这一轮同步结束。
...
云同步不是备份历史版本系统。大规模导入 Apple 健康数据、恢复备份或处理冲突前，仍建议先在“设置 → 导入与备份 → 备份与恢复”导出 JSON。
...
日级人工确认随 settings 中按日键传输；训练可选字段随 training 原组对象传输。未增加数据库表、修改远端 schema_version 或 Apple Health 协议。新客户端保存、清洗、备份与恢复保留扩展字段；恢复前拒绝非法字段并指出日期/动作/组。

同账号双客户端的字段往返已在 fakeClient/fakeDb 测试中覆盖，真实双设备仍须使用隔离账号验收。旧客户端不能编辑新训练字段，旧 normalizeSession 仍可能在保存时丢弃字段，因此多设备使用前应统一升级。旧客户端改过饮食后遗留的完整声明，由条目签名失配降为未确认。

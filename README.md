...
当前版本：**v1.7.1**（补 39 道家常炒菜与汤；修复身体信息不合格会让应用起不来、推荐份量漏出浮点数、训练建议刷屏）
...
内置 1010 种常见食物，覆盖家常菜、日常外卖（黄焖鸡米饭、水煮肉片、螺蛳粉、麻辣香锅……）、
早餐面点、地方主食、火锅烧烤、便利店食品、常见果蔬菌菇，以及 **166 种连锁餐饮单品**：
...
九田家、丰茂烤串和破店肥哈。安又胖收录厚切黑猪五花、原切牛肋排卷、招牌秘制牛肉、
...
「数据」页会把同步来的数字翻译成结论与做法，而不只是画条线。
近期概览、健康数据解读、当前每日目标、趋势图和逐日明细都在这一页——
「我最近怎么样」和「我在往哪走」本来就是同一个问题。

统计图一次只画一张，上面一排按钮切换；没有数据的选项变淡但仍可点开。
所有图统一使用同一套折线图和横轴窗口；饮食漏记的日期会断线，不会跨过空白日
虚构连续数据。7 天视图可点任意日期显示当天数值。
每张图下方给的是走势解读和该怎么办（`js/core/trend-reading.js`，纯函数、可单测），
统计口径说明收在卡片右上角的信息按钮里。
...
- **起手组合** 按三个不同的动作模式搭，每一个都补上前面练不到的地方。

只给能从「动作构成」本身看出来的结论。训练量、强度、周期要结合个人情况，这里不猜。

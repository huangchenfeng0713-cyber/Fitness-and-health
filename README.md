...
当前版本：**v1.4.0**（自选配料清补凉、地方小吃与果汁扩充、趋势折线统一版）

同步 Apple 健康数据 + 记录每日饮食 → 按你的身体信息实时判断热量与蛋白摄入，
并根据**此刻剩余的预算**给出「现在该吃什么」和「现在别碰什么」。

纯前端、零依赖、零构建。健康与饮食数据只存在你自己的浏览器里（IndexedDB），没有账号，
不会上传到应用服务器；首次加载、版本更新以及你主动打开 GitHub 反馈页时仍会访问网络。
...
内置 942 种常见食物，覆盖家常菜、日常外卖（黄焖鸡米饭、水煮肉片、螺蛳粉、麻辣香锅……）、
...
包括红牛、东鹏特饮、战马、乐虎、Monster、佳得乐、POWERADE、宝矿力、脉动、尖叫等常见搜索词。
液体按 ml 记录；含咖啡因条目会按所选容量显示整份咖啡因，高咖啡因产品会额外提醒。

地方小吃新增福鼎肉片、土家酱香饼、福州锅边糊、福州肉燕、沙县扁肉等；饮品新增苹果汁、
...
趋势页所有统计图统一使用同一套折线图和横轴窗口；饮食漏记的日期会断线，不会跨过空白日
虚构连续数据。7 天视图可点任意日期，让各图同步显示当天数值。
...
- 健康、饮食、设置和自定义食物存在浏览器的 IndexedDB 里，**应用没有接收这些数据的后端服务器**。
- 打开网页、检查新版本和主动跳转 GitHub 反馈页会产生普通网络请求，但不会附带上述个人数据。

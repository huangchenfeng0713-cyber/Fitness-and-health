...
当前版本：**v1.7.7**（去重：同一批数字不再在两页各写一遍——今日页删掉只读的记录卡、Apple 健康瘦成一行，多日趋势归数据页的图；每日目标和动作列表默认收起）
...
（7 天 / 近一个月 / 近六个月 / 全部）；没有数据的指标标注「暂无数据」但仍可选，
点进去会说明缺什么。所有图统一使用同一套折线图和横轴窗口；饮食漏记的日期会断线，

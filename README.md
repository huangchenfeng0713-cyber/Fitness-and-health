...
当前版本：**v1.1.0**（Apple 健康高保真导入与中国食物库增强版）
...
用户针对每个指标设置的来源排序，因此无法保证与「健康」App 逐值完全一致；健康页可按 `sourceName` 设置统一覆盖顺序，
没有设置时会明确标成推断值。
...
- **TDEE**：默认按活动系数估算；一旦导入 Apple 健康能量数据，就改用设备记录的
  `静息消耗 + 活动消耗`，再结合已过去的时间与近期基线估算全天。界面会区分实测值与预计值，
  并与趋势页保持同一口径。
...
内置 666 种中国常见食物，覆盖家常菜、日常外卖（黄焖鸡米饭、水煮肉片、螺蛳粉、麻辣香锅……）、

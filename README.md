...
当前版本：**v3.14.3**（餐次与当前收支建议、饮水反馈、同义组合菜去重）
...
- **状态判定** —— 一句话说清现在是正常、要注意还是已超标，以及为什么。
  其中「这个钟点该吃到多少了」有两套口径，在**设置 → 计算与显示**里选：
  **按膳食指南**（三餐供能比：早 25~30%、午 30~40%、晚 30~35%）或
  **按我平常**（用近 14 天你自己的记录算出真实分布，不足 7 天自动回到指南并说明）。
  两套都是折线，不是匀速直线 —— 没有人从早到晚均匀地吃。

...
当前版本：**v3.26.0**（整体视觉重做：翡翠光面主卡与发光圆环、浮动玻璃底栏、圆体数字、卡片图标；数值浮层跟着圆点滚动；凌晨与未同步时的文案改成人话）

**拍营养成分表**：饮食页「自定义」里点「拍照」或「从相册选」，能量和六项营养（蛋白、脂肪、碳水、膳食纤维、糖、钠）自动填进表单。
识别在手机本地完成（tesseract.js），**照片不上传到任何地方**；第一次使用要从 CDN 下载约 6 MB 的识别引擎和中文模型，之后走缓存，一般不用再下。
千焦自动换算、「每份」自动折成每 100 克、食盐自动折成钠；机器填的格子带浅绿底，推断出来的（比如「钠」字没认清、按毫克推断）标成橙色，请对照包装核对。

本版交互和验证说明见 [饮食弹窗与日期回顾](docs/DIET_INTERACTIONS.md)。

摄入趋势和训练统计口径见 [摄入趋势与训练覆盖](docs/INTAKE_TRAINING.md)。沿用 v3.12.0 的固定圆环计划尺度、三餐阶段参照和现有食物推荐。

同步 Apple 健康数据 + 记录每日饮食 → 按你的身体信息实时判断热量与蛋白摄入，
并根据**此刻剩余的预算**给出「现在该吃什么」。

纯静态、零构建。默认本地模式下，健康与饮食数据只存在你自己的浏览器里（IndexedDB）；
站点配置 Supabase 后，可选择邮箱密码或 Google 登录，把数据同步到当前账号独有的云端空间。
未登录不会上传个人数据；首次加载、版本更新、第一次使用「拍营养成分表」（下载识别引擎，照片本身不上传）以及你主动打开 GitHub 反馈页时仍会访问网络。
...
设计与功能上还没做的事记在 [`docs/改进建议.md`](docs/改进建议.md)。
...
或 SyncIdentifier/SyncVersion 的样本会按身份去重；同步记录只保留最高版本。多来源累计量按实际样本区间边界解析，
...
- **目标**：减脂 / 维持 / 增肌 + 每周速率（kg/周），1 kg 脂肪按 7700 kcal 折算。
  减重持续超过约 1% 体重/周、增重超过增肌期常用的 0.25%–0.5%/周时会提示风险，
  但不把参考线冒充诊断线；只有明显疑似填错的量级才拒绝保存。每日热量调整仍受计划护栏约束。
...
内置 1225 种常见食物（含 284 道菜肴、210 项连锁快餐），并能按基础食材生成受控的家常组合菜；覆盖家常菜、日常外卖（黄焖鸡米饭、水煮肉片、螺蛳粉、麻辣香锅……）、
早餐面点、地方主食、火锅烧烤、便利店食品、常见果蔬菌菇，以及 **206 种连锁餐饮单品**：
肯德基、麦当劳、汉堡王、德克士、塔斯汀、华莱士、必胜客、赛百味、老乡鸡、沙县小吃、
萨莉亚、和府捞面、吉野家、海底捞、刘文祥麻辣烫；蜜雪冰城、喜茶、奈雪的茶、茶百道、古茗、沪上阿姨、一点点、
...
海底捞另收录牛五花牛肉酱拌饭、麻辣磷虾盖码饭、番茄牛腩饭、香辣肉酱虾仁盖码饭、
红葱脆骨蛋炒饭与黑松露鱼籽牛肉饭；刘文祥麻辣烫可按黄细面、牛筋面、豆泡、蔬菜、丸类和麻酱骨汤逐项调量。
...
不会把推算值伪装成官方数据。搜索、份量、推荐和饮食记录统一使用低强调的「估算」标签；
估算依据、资料核对日期与主要误差来源集中在卡片或份量面板的信息按钮中，不在列表里重复铺说明。
...
还可以按包装营养成分表自建食物、沿用自己上次确认的份量，并用「和昨天一样」一键复制。
...
- **运动量**在日历覆盖率足够时折算设备记录时长；只有在这些分钟主要达到中等强度的条件下，才与 WHO 每周至少 150 分钟参考对照；明确的零运动日计为 0，缺测日不冒充 0
- **设备数据边界**：活动能量视为估算值，静息心率优先和个人基线比较，BIA 体脂只看同条件下的长期趋势；不会用单点读数诊断健康状态
- **睡眠**以 AASM 成人规律睡眠不少于 7 小时为核心参考；超过 9 小时不会自动判定为更好或异常，
  会结合白天困倦和所处情境提示复核；标准差只描述“睡眠时长波动”，不冒充入睡/起床规律
- **静息心率**结合个人趋势提示复核；单凭平均值或短期上升不作疾病判断，有症状时建议就医
- **体重趋势**用最小二乘拟合。减重持续 >1% 体重/周或增重 >0.5%/周会按不同风险提醒；
  只有跨度至少 28 天且称重点足够时，才给出不超过 ±250 kcal 的小步试调，并明确这只是能量换算估计
...
- **状态判定** —— 一句话说清现在是正常、要注意还是已超标，以及为什么。
  「这个钟点该吃到多少了」统一使用固定三餐参照：早 30%、午 40%、晚 30%。
  餐次窗口内平滑推进、两餐之间保持；设置中只读展示「参照膳食」。不再从历史记录学习个人时间或餐量分布。
...
js/
  ...
  core/
    ...
    eating-rhythm.js            进食节奏：这个钟点按计划应该吃到多少了
    energy-ring.js              今日热量跑道：绿摄入、黄消耗、当天锁定尺子
...
- 云同步不等于历史版本备份。大规模导入、恢复或换设备前，仍建议到「设置 → 导入与备份 → 备份与恢复」导出完整 JSON。

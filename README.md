...
```bash
npm test               # node --test，57 个用例，覆盖营养计算 / 建议引擎 / 健康数据解析 / 食物库自洽性
```
...
内置 285 种中国常见食物（主食 42 / 菜肴外卖 42 / 蔬菜 36 / 肉禽 27 / 水果 24 /
零食 22 / 饮品 21 / 水产 18 / 豆制品 15 / 乳制品 13 / 坚果 13 / 蛋类 8），
每种带每 100 g 的七项营养和常用份量。支持中文名、俗称与拼音搜索（「西红柿」「稀饭」「jixiong」都能命中）、
按包装营养成分表自建食物、常吃快捷入口、「和昨天一样」一键复制。
